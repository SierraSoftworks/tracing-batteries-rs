...
impl Medama {
    ...
    /// Configures the page URL which should be used for the analytics event.
    ///
    /// This method allows you to specify the page URL that will be sent to
    /// the Medama server as part of the analytics data. If not set, `/` will
    /// be used by default.
    ///
    /// ## Example
    /// ```no_run
    ...
    /// session.shutdown();
    /// ```
    pub fn with_page<S: Into<Cow<'static, str>>>(mut self, page: S) -> Self {
